...
class Callback implements CallbackInterface
{
    ...
    {
        if (0 < count($this->specs[$clause]) && !array_intersect($value, $this->specs[$clause])) {
            return false;
        }

        if (0 < count($this->specs['excluded_'.$clause]) && array_intersect($value, $this->specs['excluded_'.$clause])) {
            return false;
        }
